...
func (r *AppReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	for _, val := range envs {
		env := val["env"]
		//r.Log.Info("env", "env", env)
		ctrl.Log.Info("env", "env", env)

		deploymentName := req.Name + "-" + env
		//req.NamespacedName.Name = deploymentName
		...
		if err := r.Client.Get(ctx, req.NamespacedName, oldDeploy); err != nil {
			...
			if errors.IsNotFound(err) {
				//创建deployment
				if err := r.Client.Create(ctx, deployment.New(instance, env)); err != nil {
					return ctrl.Result{}, err
				}
				//创建service
				if err := r.Client.Create(ctx, service.New(instance, env)); err != nil {
					return ctrl.Result{}, err
				}
			} else {
				return ctrl.Result{}, err
